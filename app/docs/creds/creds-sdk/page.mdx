...
<Callout type="info">
  The **Ezrah Creds SDK** is a TypeScript package that simplifies the creation, issuance, and verification of Verifiable Credentials (VCs), along with DID management, webhook integrations, and analytics.
</Callout>

<Callout type="default">
  **Current Version:** `@coincord/ezrah-creds-sdk@1.0.0-alpha.14`
</Callout>
...
<Callout type="warning">
  **Important:** Ezrah (Coincord) libraries are hosted on GitHub, not npm. Add the following to your `.npmrc` file:
</Callout>
...
<Callout type="default">
  **Generate API Keys:** You can create new API keys from your Ezrah Dashboard
</Callout>
...
Sub issuers are secondary credential issuers that operate under your organization's authority, enabling delegated credential issuance while maintaining trust and governance. Each sub issuer has its own decentralized identifier (DID) but inherits the parent organization's permissions and authentication.

### Create a Sub Issuer

Create a new sub issuer under your organization:

```ts
const subIssuer = await ezrah.createSubIssuer({
  name: "University Registrar",
  description: "Academic credential issuer for degree certificates",
  domain: "registrar.university.edu"
});

console.log(`Sub issuer created with DID: ${subIssuer.did}`);
```

**Response:**
```typescript
{
  did: string;        // Decentralized Identifier for the sub issuer
...
Retrieve all sub issuers under your organization:

```ts
const subIssuers = await ezrah.subIssuers();
```

**Response:**
```typescript
SubIssuer[] // Array of sub issuer objects
```

### Issue Credentials with Sub Issuers

When issuing encrypted SD-JWT credentials, you can specify which sub issuer should be used:
...
const credential = await ezrah.issueEncryptedSDJWT(
  claims,
  ['studentId', 'degree'], // Selective disclosure fields
  [recipientPublicKey],
  {
    policy_control: true,
    sub_issuer_did: "did:ezrah:registrar123" // Sub issuer DID
  }
);
```

**Benefits of Sub Issuers:**
- **Delegated Authority**: Issue credentials under organizational hierarchy
- **DID-based Identity**: Each sub issuer maintains its own decentralized identifier
- **Selective Disclosure**: Full support for privacy-preserving SD-JWT credentials
- **Organizational Structure**: Clear governance and trust relationships

<Callout type="info">
  Sub issuers inherit authentication and permissions from the parent organization. All API operations use the same organizational API key.
</Callout>
...
const result = await ezrahCredsSDK.issueEncryptedSDJWT(
  ...
  {
    policy_control: false, // default
    sub_issuer_did: "did:ezrah:registrar123" // Optional: specify sub issuer
  }
);
```
